...
Business stages explain user intent. The script phases implement connector context and authentication, provider publication, consumer negotiation, transfer and EDR retrieval, then download and validation. See [Architecture](../architecture.md) and [Execution phases](../execution-phases.md).
...
| Act | Consumer-side operator on behalf of the functional use case | Retrieve and use the selected resource | Start transfer, retrieve an EDR, and execute phase4 through the Data Plane | The consumer connector obtains the EDR; the Data Plane resolves the provider data address | A non-empty resource is downloaded and semantically validated | Validated | [WFS](../flows/wfs.md), [SPARQL](../flows/sparql.md), and [Ingestion API](../flows/ingestion-api.md) |
| Result | City Council stakeholder and operator | Retain an understandable result and traceability | Review the validated output, sanitized summary, manifest, byte count, and SHA-256 concept | The execution correlates publication, agreement, transfer, and download under one run | Verified technical result with run-level evidence | Validated | [Evidence and traceability](../evidence-and-traceability.md) |
...
- downloads through the Data Plane;
- validates the selected representation;
...
- a materialized non-empty download;
- flow-specific semantic validation;
- a manifest containing file metadata, byte count, and SHA-256;
